...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_out", type=str, default="./out", help="Output directory path")
    ...
    if args.surfaces_multiprocessing:
        multiprocessing.set_start_method("spawn", force=True)
        fn_process = process_multiprocessing

    os.makedirs(args.path_out, exist_ok=True)

    os.makedirs("{}/unclipped".format(args.path_out), exist_ok=True)
    os.makedirs("{}/clipped".format(args.path_out), exist_ok=True)
    os.makedirs("{}/topo".format(args.path_out), exist_ok=True)

    # ============================ load points ============================
    path_in = input("Enter path to the .xyzc model file: ")
    assert os.path.exists(path_in), "Input points could not be accessed"
    ...
    out_meshes = fn_process(args, uniq_labels, points, labels, device)

    # ============================ save unclipped meshes ============================
    print("Saving unclipped meshes...")
    pm_meshes = save_unclipped_meshes(
        out_meshes, color_list, "{}/unclipped/mesh.ply".format(args.path_out)
    )

    # ============================ save clipped meshes ==============================
    print("Saving clipped meshes...")
    clipped_meshes = save_clipped_meshes(
        pm_meshes, out_meshes, color_list, "{}/clipped/mesh.ply".format(args.path_out)
    )

    # ============================ get edges and corners ============================
    print("Saving topology (edges and corners)...")
    save_topology(clipped_meshes, "{}/topo/topo.json".format(args.path_out))
